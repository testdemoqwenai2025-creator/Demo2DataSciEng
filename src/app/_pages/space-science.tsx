  ...
  return (
    <div className="space-y-8">
      ...
      <DeeperThoughtSection pageTitle="Space Science">
        ...
        <DeeperThought title="Kepler's third law IS the power law — and it's universal" connectedTo="ADR-001 (platform architecture)">
          <p>{"Kepler's T² = (4π²/GM)·a³ IS a power law: T ∝ a^(3/2). Power laws appear everywhere: in wealth (Pareto), in city sizes (Zipf), in earthquake magnitudes (Gutenberg-Richter), in internet topology (scale-free networks). The SAME mathematical form (y ∝ x^α) governs planetary orbits, wealth distribution, and network degree. Kepler discovered a power law in 1619 — 250 years before Pareto. The math IS the connection between 17th-century astronomy and 20th-century economics."}</p>
        </DeeperThought>
        <DeeperThought title="Transit photometry IS FFT — detecting planets from periodic brightness dips" connectedTo="ADR-051 (living-equation pages)">
          <p>{"Kepler/TESS detect exoplanets by measuring star brightness over time. When a planet transits, brightness dips by ~0.01% (for Earth-sized planet around Sun-sized star). The transit IS periodic (orbital period). Detecting it IS FFT: the light curve's frequency spectrum has a peak at 1/orbital_period. The SAME FFT that separates C-major chord notes detects exoplanets. The math (FFT) IS the bridge between audio and astronomy."}</p>
        </DeeperThought>
        <DeeperThought title="The Drake equation IS Bayesian — and it's the right framework for SETI" connectedTo="ADR-007 (Bayesian methods)">
          <p>{"Drake's equation N = R* · fp · ne · fl · fi · fc · L estimates the number of communicative civilisations. Each factor IS a prior probability. The equation IS Bayes' theorem applied to existential risk: P(civilisation | observation) = P(observation | civilisation) · P(civilisation) / P(observation). The factors are uncertain (priors range over orders of magnitude). The Bayesian framework IS the right one: it quantifies uncertainty, updates with new data (exoplanet counts from Kepler), and doesn't pretend to know more than it does."}</p>
        </DeeperThought>
        <DeeperThought title="Gaia's 1.8 billion stars IS the largest PCA ever — and it found the Milky Way's history" connectedTo="ADR-034 (ESM-2 + AlphaFold2)">
          <p>{"Gaia DR3 measures positions, proper motions, and radial velocities for 1.8 billion stars. PCA on this 1.8B × 30 matrix reveals the Milky Way's structure: spiral arms, globular clusters, tidal streams from disrupted dwarf galaxies. The SAME PCA that recovers Out-of-Africa from 1000-Genomes recovers the Milky Way's merger history from Gaia. The top principal components ARE the galactic structure — just as the top PCs of genetic variation ARE the ancestral migrations. SVD IS the universal decomposer, from DNA to galaxies."}</p>
        </DeeperThought>
