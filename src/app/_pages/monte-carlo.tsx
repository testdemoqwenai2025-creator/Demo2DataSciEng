  ...
  return (
    <div className="space-y-8">
      ...
      <DeeperThoughtSection pageTitle="Monte Carlo">
        ...
        <DeeperThought title="MC IS to data what Datadog was to software — and the pattern IS identical" connectedTo="ADR-050 (fold-section architecture)">
          <p>{"Before Datadog (2010), software monitoring was ad-hoc (Nagios + custom scripts). Datadog standardised monitoring with agents + cloud + dashboards + ML baselines. MC is doing the same for data: agents (monitors on tables), cloud (SaaS), dashboards (anomaly alerts), ML baselines (expected ranges). The pattern (agent + cloud + dashboard + ML) IS identical. The artifact differs (data vs software). The math (statistical monitoring) stays. The implementation (MC vs Datadog vs New Relic) changes. The pattern absorbs the domain."}</p>
        </DeeperThought>
        <DeeperThought title="MC's ML baselines IS time-series forecasting — and it's the right tool" connectedTo="ADR-022 (pgvector for variant embeddings)">
          <p>{"Monte Carlo's ML baselines forecast expected data metrics (row counts, freshness, null rates) using time-series models (ARIMA, Prophet, LSTM). When actual deviates from forecast by >3σ, alert. This IS the SAME pattern as financial VaR: expected = μ, actual = z, alert if |z - μ| > 3σ. The 3σ threshold IS the 99.7% confidence interval. The math (Gaussian anomaly detection) IS the connection between data observability and risk management. VaR IS data observability for finance; MC IS financial VaR for data."}</p>
        </DeeperThought>
        <DeeperThought title="Freshness monitoring IS exponential distribution — and it's the right model" connectedTo="ADR-055 (cross-disciplinary scope)">
          <p>{"Monte Carlo's freshness monitor checks: 'when was the last data update?' If the gap exceeds the expected interval, alert. The gap between updates follows an exponential distribution (memoryless — the same property as Poisson arrivals). The expected gap IS 1/λ (mean inter-arrival time). The alert threshold IS P(gap > T) = e^{-λT} < 0.05 (5% false alarm). This IS the SAME math as radioactive decay (half-life = ln(2)/λ) and server timeout (P(timeout) = e^{-λT}). The exponential distribution IS the universal model for 'time until next event.'"}</p>
        </DeeperThought>
        <DeeperThought title="MC's lineage IS the dependency graph — and it's the SAME as git blame" connectedTo="ADR-050 (fold-section architecture)">
          <p>{"Monte Carlo's lineage feature traces data dependencies: table A depends on table B depends on table C. When table C breaks, MC alerts all downstream tables. This IS the SAME pattern as git blame (which file caused this bug?) and Kubernetes service mesh (which service depends on which?). The lineage graph IS a DAG (directed acyclic graph) — the SAME structure as dbt's DAG, Airflow's DAG, and Make's dependency graph. The math (DAG traversal) stays; the implementation (MC lineage vs dbt manifest vs OpenLineage) changes."}</p>
        </DeeperThought>
