  ...
  return (
    <div className="space-y-8">
      ...
      <DeeperThoughtSection pageTitle="Numpy Scipy">
        ...
        <DeeperThought title="BLAS and LAPACK are NumPy's hidden engine — and they're 50 years old" connectedTo="ADR-022 (pgvector for variant embeddings)">
          <p>{"np.linalg.svd calls LAPACK's DGESDD, which was written in Fortran in the 1970s. The same Fortran code runs every SVD on the planet — from 1000-Genomes PCA to Fama-French factor analysis to JPEG compression. NumPy is a Python wrapper around 50-year-old Fortran. The math (SVD, QR, eigenvalue) is 150 years old. The Fortran implementation is 50 years old. The Python wrapper is 20 years old. Three layers of history, each surviving because the layer below it got the math right."}</p>
        </DeeperThought>
        <DeeperThought title="Broadcasting IS NumPy's killer feature — and it's pure linear algebra" connectedTo="ADR-001 (platform architecture)">
          <p>{"NumPy broadcasting (m,1)+(1,n) → (m,n) is not syntactic sugar — it's a mathematical statement about tensor rank. The operation IS outer product addition. Broadcasting makes this O(1) in memory (no actual (m,n) matrix is materialised) and O(mn) in compute. This is why NumPy is fast: it doesn't copy data, it describes the operation mathematically and lets BLAS execute it optimally. Understanding broadcasting IS understanding the difference between mathematical description and computational execution."}</p>
        </DeeperThought>
        <DeeperThought title="The C/Fortran-order debate IS the column-major vs row-major question" connectedTo="ADR-050 (fold-section architecture)">
          <p>{"NumPy defaults to C-order (row-major) because C is the dominant system language. SciPy defaults to Fortran-order (column-major) because LAPACK is Fortran. The transpose between them is a no-op in NumPy (strides), but it's a deep-copy in raw C. This is why np.asfortranarray() exists — it's a hint to LAPACK that the array is already in the right memory layout. The row-vs-column debate is 60 years old and won't be resolved — but NumPy's stride-based abstraction means the user never has to think about it."}</p>
        </DeeperThought>
        <DeeperThought title="np.linalg.svd on 1000-Genomes IS the platform's most-used call" connectedTo="ADR-051 (living-equation pages)">
          <p>{"The /living-svd page runs np.linalg.svd on a synthetic 1000-Genomes matrix in your browser via Pyodide. In production, the same call on the real 2504 × 3M matrix takes ~10 minutes on a 16-core machine with ~30 GB RAM. The browser demo uses 200 × 500 (synthetic) and takes <1 second. The scale difference is 10⁶× — but the equation is identical. The browser demo IS the production call, just smaller. That's the power of representational code: the math doesn't scale, the implementation does."}</p>
        </DeeperThought>
