  ...
  return (
    <div className="space-y-8">
      ...
      <DeeperThoughtSection pageTitle="Bioinformatics Pipelines">
        ...
        <DeeperThought title="BWA-MEM IS the Burrows-Wheeler Transform — and it's data compression" connectedTo="ADR-022 (pgvector for variant embeddings)">
          <p>{"BWA-MEM aligns reads to the reference genome using the Burrows-Wheeler Transform (BWT) — the SAME algorithm used by bzip2 for data compression. The BWT IS a permutation of the text that makes it more compressible. For alignment, the BWT enables backward search: 'does this read appear in the genome?' in O(n) time without scanning the genome. The SAME math that compresses files aligns DNA reads. The BWT IS the connection between information theory (compression) and genomics (alignment)."}</p>
        </DeeperThought>
        <DeeperThought title="The variant calling threshold IS Poisson(λ=14) — and it dictates the experiment" connectedTo="ADR-043 (AlphaMissense adoption)">
          <p>{"GATK's minimum coverage for reliable variant calling IS P(≥10×) ≥ 95%, which requires λ ≥ 14 (mean coverage). At λ=10, P(≥10×) = 42% — too many positions with insufficient reads. At λ=20, P(≥10×) = 99.9% — overkill. The math (Poisson CDF) tells the biologist the optimal sequencing depth: λ=14 for 95% confidence. This IS the SAME math that tells the SRE the optimal server capacity (Poisson(λ) arrivals, need μ > λ for stability). The math dictates the experiment — the biologist doesn't choose the depth, the Poisson does."}</p>
        </DeeperThought>
        <DeeperThought title="RNA-seq IS the negative binomial — and it's over-dispersed Poisson" connectedTo="ADR-022 (pgvector for variant embeddings)">
          <p>{"RNA-seq counts follow a negative binomial (NB) distribution — Poisson with an extra dispersion parameter. NB(μ, φ) has Var = μ + φμ² (over-dispersion: variance > mean). This IS the SAME upgrade path from Poisson to NB that applies to over-dispersed server traffic (bursty arrivals) and over-dispersed queueing (heavy-tailed service times). The negative binomial IS Poisson + a Gamma mixing layer — the SAME hierarchical model as Bayesian shrinkage. DESeq2 uses NB for differential expression testing — the SAME math that models insurance claims (compound Poisson)."}</p>
        </DeeperThought>
        <DeeperThought title="VCF IS the data contract for genomics — and it's the right abstraction" connectedTo="ADR-001 (platform architecture)">
          <p>{"The VCF (Variant Call Format) IS a data contract for genetic variants: CHROM, POS, ID, REF, ALT, QUAL, FILTER, INFO. Each field has a type, a meaning, and a validation rule. This IS the SAME pattern as a database schema (CREATE TABLE) or an Avro schema (JSON + types). The VCF IS the API between the variant caller (BWA+GATK) and the annotator (VEP) and the analyst (PLINK). The contract (typed schema) stays; the implementation (VCF vs BCF vs Parquet) changes. The pattern IS data engineering applied to genomics."}</p>
        </DeeperThought>
