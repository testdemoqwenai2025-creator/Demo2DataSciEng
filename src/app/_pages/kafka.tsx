  ...
  return (
    <div className="space-y-8">
      ...
      <DeeperThoughtSection pageTitle="Kafka">
        ...
        <DeeperThought title="Kafka's consumer groups IS load-balanced subscription — and it's the right model" connectedTo="ADR-050 (fold-section architecture)">
          <p>{"Kafka's consumer group protocol: each partition is assigned to exactly one consumer in the group. This IS load-balanced subscription — the SAME pattern as Kubernetes' service discovery (each pod gets traffic from the service). The partition IS the unit of parallelism. The consumer IS the worker. The group IS the service. The rebalance (when a consumer joins/leaves) IS the SAME as Kubernetes' pod rescheduling. The pattern (partition + consumer + rebalance) stays; the implementation (Kafka vs Pulsar vs SQS) changes."}</p>
        </DeeperThought>
        <DeeperThought title="Kafka's exactly-once IS idempotent producer + transactional consumer — and it's hard" connectedTo="ADR-013 (Delta Lake)">
          <p>{"Kafka's exactly-once semantics (EOS) requires: (1) idempotent producer (dedup by sequence number), (2) transactional consumer (read-committed isolation), (3) transaction coordinator (2-phase commit). This IS the SAME pattern as Delta Lake's ACID: write to a log, commit atomically, readers see committed state. The 2PC (two-phase commit) IS the SAME protocol as XA transactions in databases. The math (idempotent + atomic + isolation) stays; the implementation (Kafka transactions vs Delta commits vs DBMS 2PC) changes."}</p>
        </DeeperThought>
        <DeeperThought title="Kafka IS Poisson — and that's why it scales" connectedTo="ADR-055 (cross-disciplinary scope)">
          <p>{"Kafka topic arrivals follow Poisson(λ) — the SAME distribution that models sequencing reads and radioactive decay. The consumer processes events at rate μ. If λ > μ, the queue grows unboundedly (backlog). If λ < μ, the consumer is idle (waste). The utilisation ρ = λ/μ determines the system's behaviour. This IS the SAME queueing theory that Erlang used for telephone exchanges in 1909. Kafka IS Erlang's telephone exchange, just for data instead of calls. The math (Poisson + service rate + utilisation) stays; the technology (Kafka vs Pulsar vs Kinesis) changes."}</p>
        </DeeperThought>
        <DeeperThought title="Kafka's partition IS the shard — and it's the scalability primitive" connectedTo="ADR-022 (pgvector for variant embeddings)">
          <p>{"Kafka partitions by key (hash(key) mod N). Each partition is a separate log on a separate broker. This IS the SAME sharding that databases use (hash(shard_key) mod N). The partition IS the shard. The broker IS the shard server. The consumer IS the shard client. The rebalance IS the reshard. The pattern (hash + shard + rebalance) IS identical to MongoDB's sharding, Cassandra's vnode, and DynamoDB's partition key. The math (consistent hashing) stays; the implementation (Kafka vs Cassandra vs DynamoDB) changes."}</p>
        </DeeperThought>
