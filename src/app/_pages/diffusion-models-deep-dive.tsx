  ...
  return (
    <div className="space-y-8">
      ...
      <DeeperThoughtSection pageTitle="Diffusion Models Deep Dive">
        ...
        <DeeperThought title="The score function IS the gradient of the log-density — and it's the universal object" connectedTo="ADR-034 (ESM-2 + AlphaFold2)">
          <p>{"The score function ∇log p(x) tells you: 'which direction increases the probability density?' Following the score IS gradient ascent on p(x) — climbing toward the mode. This IS the SAME math as gradient descent (θ(t+1) = θ(t) - η∇L(θ)), just applied to the data distribution instead of the loss landscape. The score IS the gradient of the data distribution; the loss gradient IS the gradient of the model. The math (steepest ascent/descent) IS the same."}</p>
        </DeeperThought>
        <DeeperThought title="DDPM IS the discrete version — and it's the practical implementation" connectedTo="ADR-027 (diffusion models)">
          <p>{"DDPM (Ho et al. 2020) discretizes the continuous SDE into T steps (T=1000). Each step: x_{t-1} = μ(x_t, t) + σ(t)·z. The network predicts μ (the mean). This IS the Euler-Maruyama method for SDEs — the SAME integrator that simulates GBM for financial Monte Carlo. DDPM IS Euler-Maruyama applied to the reverse SDE. The math (discretize SDE + predict drift) IS the same. The application (image generation vs option pricing) differs."}</p>
        </DeeperThought>
        <DeeperThought title="Classifier-free guidance IS the conditional generation trick — and it's the insight" connectedTo="ADR-027 (diffusion models)">
          <p>{"Classifier-free guidance generates: ε_pred = (1+w)·ε_cond - w·ε_uncond. The w (guidance scale) controls how much the generation follows the condition. w=0: unconditioned (random). w=1: standard. w=7.5 (typical): strong guidance. This IS the SAME pattern as temperature in softmax: T=0 → deterministic, T=∞ → uniform. Classifier-free guidance IS temperature for conditional generation — the math (interpolation between conditional and unconditional) IS the same."}</p>
        </DeeperThought>
        <DeeperThought title="Diffusion vs GANs IS the likelihood-based vs adversarial debate — and likelihood wins" connectedTo="ADR-050 (fold-section architecture)">
          <p>{"GANs (2014) generate via adversarial training (generator vs discriminator). Diffusion (2020) generates via likelihood maximization (reverse SDE). GANs are fast (1 forward pass) but unstable (mode collapse). Diffusion is slow (1000 steps) but stable (covers all modes). The trade-off (speed vs stability) IS the same as deterministic vs stochastic. The math (min-max game vs score matching) differs; the goal (generate realistic data) IS the same. Diffusion won because stability IS more valuable than speed."}</p>
        </DeeperThought>
