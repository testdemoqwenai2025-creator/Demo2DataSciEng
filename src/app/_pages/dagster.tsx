  ...
  return (
    <div className="space-y-8">
      ...
      <DeeperThoughtSection pageTitle="Dagster">
        ...
        <DeeperThought title="Dagster's software-defined assets ARE git for data — and that's the right UX" connectedTo="ADR-050 (fold-section architecture)">
          <p>{"An SDA IS a versioned, lineage-tracked artifact — like a Git commit but for data. The SDA knows: what produced it (upstream assets), when it was last materialised, what version of the code produced it, and what downstream assets depend on it. This IS the SAME metadata that Git tracks for code (author, timestamp, parent, diff). The SDA IS a Git commit for data. The pattern (versioned + lineage + reproducible) stays; the implementation (Dagster SDA vs Delta log vs dbt manifest) changes."}</p>
        </DeeperThought>
        <DeeperThought title="Dagster's config system IS dependency injection — and it's the right pattern" connectedTo="ADR-050 (fold-section architecture)">
          <p>{"Dagster's config system (YAML/Python config injected into ops/assets at runtime) IS dependency injection — the SAME pattern that Spring (Java 2003) and FastAPI (Python 2018) use. The op doesn't hardcode its config — it receives it from the runtime. This means: the same op can run in dev (small data, debug=True) and prod (large data, debug=False) with different configs. The pattern (DI) stays; the implementation (Dagster config vs Spring beans vs Kubernetes ConfigMaps) changes."}</p>
        </DeeperThought>
        <DeeperThought title="Dagster's sensor IS the event-driven trigger — and it's the right abstraction" connectedTo="ADR-022 (pgvector for variant embeddings)">
          <p>{"Dagster sensors trigger pipelines based on external events (new file in S3, new Kafka message, dbt run complete). This IS the SAME pattern as Kubernetes' CronJob + Eventing (knative-eventing). The sensor IS the event source. The pipeline IS the handler. The trigger IS the binding. The pattern (event source + handler + binding) IS identical to AWS Lambda (event source mapping). The math (event-driven) stays; the implementation (Dagster sensor vs Lambda trigger vs Airflow sensor) changes."}</p>
        </DeeperThought>
        <DeeperThought title="Dagster vs Airflow IS the typed vs untyped debate — and types win" connectedTo="ADR-050 (fold-section architecture)">
          <p>{"Airflow's DAG IS a task graph (untyped — tasks are just Python functions). Dagster's asset graph IS a data graph (typed — assets know their type and schema). The typed vs untyped debate IS the SAME as TypeScript vs JavaScript: types catch errors early, enable better tooling (auto-complete, type checking, lineage), and prevent the 'wrong format' bug. Dagster IS TypeScript for data. Airflow IS JavaScript — it works, but you'll spend 20% of your time debugging type errors that a compiler would have caught."}</p>
        </DeeperThought>
