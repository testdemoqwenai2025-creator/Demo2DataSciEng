...
└──────────────────────────────────────────────────────────────────────────┘`} />
      ...
      <DeeperThoughtSection pageTitle="Systems Biology">
        <DeeperThought title="The cell IS the original distributed system" connectedTo="ADR-001 (platform architecture)">
          <p>{"FBA is LP for resource allocation — the SAME math Amazon uses for warehouse inventory. PPI networks are distributed systems with hubs = load balancers. Whole-cell simulation IS microservices architecture — Karr 2012's 28 sub-models are 28 microservices integrated by message passing. The cell's chromosome IS a configuration file. The cell cycle IS the deployment cycle (build → test → release → scale → divide). Evolution has been load-testing the cell for 4 billion years. Biology IS the original cloud."}</p>
        </DeeperThought>
        <DeeperThought title="FBA IS supply chain optimisation — Dantzig's Simplex 1947" connectedTo="ADR-022 (pgvector for variant embeddings)">
          <p>{"Flux Balance Analysis (FBA) maximises biomass reaction subject to S·v=0 (mass balance). This IS the same LP that Amazon solves for warehouse inventory: maximise customer satisfaction subject to supply constraints. Dantzig invented the Simplex algorithm in 1947 for the US Air Force supply chain — and the same algorithm now solves metabolic network optimisation. The stoichiometric matrix S IS a bill of materials. The flux vector v IS the production rate. LP is invariant to domain — oil, packets, or glucose."}</p>
        </DeeperThought>
        <DeeperThought title="PPI networks ARE service dependency graphs — knockout = chaos engineering" connectedTo="ADR-050 (fold-section architecture)">
          <p>{"Hub proteins (high PageRank) are load balancers — many other proteins depend on them, like a database that many microservices call. Essentiality prediction via PageRank IS the same analysis that Netflix does for service dependency graphs: which services would cause a cascading failure if down? Knockout screens (knock out each gene, see which kill the cell) IS chaos engineering (kill each service, see which break the system). ~10% of genes are essential = ~10% of microservices are critical for any large distributed system. The same mathematical structure applies to both."}</p>
        </DeeperThought>
        <DeeperThought title="MOFA+ IS matrix factorisation — the SAME math as recommender systems" connectedTo="ADR-022 (pgvector for variant embeddings)">
          <p>{"Multi-Omics Factor Analysis (MOFA+) decomposes multi-omics data X_m = W_m·Z + ε_m via alternating optimisation. This IS the same matrix factorisation that powers Netflix's recommender system: users × items → latent factors × users. In MOFA+: omics × samples → latent factors × samples. The math is identical — only the domain differs. The cell IS a recommender system where the 'items' are genes and the 'users' are conditions."}</p>
        </DeeperThought>
        <DeeperThought title="The 2nd law of thermodynamics IS the arrow of time — and it applies to the cell" connectedTo="ADR-055 (cross-disciplinary scope)">
          <p>{"Entropy always increases — in a gas, in a message, in a population. The cell fights the 2nd law by consuming energy (ATP hydrolysis) to maintain order (low entropy). When the cell stops consuming energy, it dies — entropy wins. This IS the same principle as a database: without active maintenance (vacuum, reindex, replication), entropy (corruption, drift, data loss) increases. The cell IS a self-maintaining database that fights the 2nd law via metabolic energy."}</p>
        </DeeperThought>
