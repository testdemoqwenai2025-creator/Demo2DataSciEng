  ...
  return (
    <div className="space-y-8">
      ...
      <DeeperThoughtSection pageTitle="Computational Biology">
        ...
        <DeeperThought title="The force field IS a machine learning model — trained on quantum chemistry" connectedTo="ADR-034 (ESM-2 + AlphaFold2)">
          <p>{"AMBER's force field (bond, angle, dihedral, van der Waals, electrostatic) is a parametrised function F(positions) → energy. The parameters are fit to quantum chemistry calculations (DFT on small molecules). This IS supervised learning: input = molecular geometry, output = energy, model = force field. Neural network potentials (NNP) replace the hand-crafted force field with a neural network — same inputs, same outputs, different model class. The force field IS the original ML model for molecular simulation."}</p>
        </DeeperThought>
        <DeeperThought title="AMBER's O(N^2) force calculation IS the scaling bottleneck — and PME fixes it" connectedTo="ADR-022 (pgvector for variant embeddings)">
          <p>{"For N=10^6 atoms, the pairwise force calculation is 10^12 operations per step — infeasible. Particle Mesh Ewald (PME) splits the force into short-range (direct, O(N)) and long-range (FFT-based, O(N log N)) components. The FFT in PME IS the same FFT that separates C-major chord notes — just applied to charge density instead of audio. The SAME equation (X[k] = Σ x[n] e^{-2πikn/N}) appears in MD (PME long-range forces), audio (frequency analysis), and cryo-EM (3D reconstruction). FFT IS the universal decomposition."}</p>
        </DeeperThought>
        <DeeperThought title="Enhanced sampling IS the multi-armed bandit — explore vs exploit" connectedTo="ADR-006 (RL agentic)">
          <p>{"Metadynamics, replica exchange, and umbrella sampling all solve the same problem: the simulation is stuck in a local free-energy minimum (exploit), but we need to explore other minima. Replica exchange (run N simulations at different temperatures, swap) IS the multi-armed bandit: explore high-T replicas (explore) + exploit low-T replicas (exploit). The SAME explore-vs-exploit trade-off that governs RL agents governs enhanced sampling. Thompson sampling IS Bayesian enhanced sampling."}</p>
        </DeeperThought>
        <DeeperThought title="The AMBER/GROMACS/Havok/NASA triangle IS the multi-disciplinary elegance" connectedTo="ADR-055 (cross-disciplinary scope)">
          <p>{"AMBER simulates proteins. GROMACS simulates lipids. Havok simulates ragdolls. NASA JPL simulates spacecraft. All use Verlet integration. All conserve energy (symplectic property). All are O(N^2) pairwise force (unless PME-accelerated). The SAME code, different parameters. A biochemist, a game developer, and an aerospace engineer are using the SAME algorithm — and none of them knows it. The platform exists to show them the connection."}</p>
        </DeeperThought>
