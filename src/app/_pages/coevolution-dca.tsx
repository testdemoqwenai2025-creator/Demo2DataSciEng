...
└──────────────────────────────────────────────────────────────────────────┘`} />
      ...
      <DeeperThoughtSection pageTitle="Coevolution Dca">
        ...
        <DeeperThought title="DCA vs Attention IS the precision-matrix vs dot-product debate" connectedTo="ADR-034 (ESM-2 + AlphaFold2)">
          <p>{"DCA computes the precision matrix (inverse covariance) — O(N^3) in sequence length. Attention computes QK^T (dot product) — O(N^2). Both find contacts, but attention is faster. The trade-off: DCA is mathematically rigorous (precision matrix captures direct correlations); attention is empirically powerful (learns from data). The math (precision matrix vs dot product) IS the same debate as Lasso vs neural networks (structured vs unstructured). AlphaFold2 chose attention; DCA chose precision. Both work."}</p>
        </DeeperThought>
        <DeeperThought title="The MSA IS the evolutionary dataset — and it's the training data for both DCA and Attention" connectedTo="ADR-037 (genetic materials + variant calling)">
          <p>{"Both DCA and Attention take an MSA as input. The MSA IS a matrix of amino acids (N positions × M sequences). Each row IS one organism's protein. Mutations between rows ARE the evolutionary signal. DCA computes correlations between columns (positions). Attention computes QK^T between positions. The MSA IS the dataset; DCA and Attention are two different mathematical tools applied to the same data. The dataset stays; the tool evolves."}</p>
        </DeeperThought>
        <DeeperThought title="Coevolution IS natural selection's fingerprint — and it's the contact map" connectedTo="ADR-043 (AlphaMissense adoption)">
          <p>{"When two residues are in physical contact, a mutation in one is compensated by a mutation in the other (to maintain the fold). Natural selection constrains these pairs to co-vary. Co-evolution IS this constraint's fingerprint — the correlation matrix of the MSA captures it. The contact map IS the physical structure; the correlation matrix IS the evolutionary record. They ARE the same information, viewed from different angles. Co-evolution IS the bridge between sequence and structure."}</p>
        </DeeperThought>
        <DeeperThought title="DCA's PLM (pseudo-likelihood) IS the regularized estimation — and it's the right approach" connectedTo="ADR-022 (pgvector for variant embeddings)">
