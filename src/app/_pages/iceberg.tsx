  ...
  return (
    <div className="space-y-8">
      ...
      <DeeperThoughtSection pageTitle="Iceberg">
        ...
        <DeeperThought title="Iceberg's manifest IS a B-tree for object storage — and it's the right abstraction" connectedTo="ADR-050 (fold-section architecture)">
          <p>{"Iceberg's manifest list → manifest files → data files is a 3-level B-tree for object storage. The manifest list IS the root (points to manifests). Each manifest IS a B-tree node (points to data files). Each data file IS a leaf. The tree enables data skipping: if the manifest says 'no rows matching WHERE x > 10 in this file', skip it. This IS the same pruning as Snowflake's micro-partitions and Delta's Z-ORDER. The pattern (tree-based data skipping) stays; the implementation (Iceberg manifests vs Delta logs vs Snowflake micro-partitions) changes."}</p>
        </DeeperThought>
        <DeeperThought title="Iceberg's schema evolution IS Avro + projection — and it's free" connectedTo="ADR-001 (platform architecture)">
          <p>{"Iceberg stores each column in its own file (columnar). Adding a column = add a new file — no rewrite. Renaming a column = update the metadata — no rewrite. Dropping a column = stop reading it — no rewrite. This IS the same as Avro's schema evolution (add fields without rewriting). The columnar layout IS the enabler: each column is independent, so schema changes don't affect other columns. The math (column independence) IS the insight; the format (Iceberg vs Delta vs Hudi) is the implementation."}</p>
        </DeeperThought>
        <DeeperThought title="Iceberg's branchless merge IS Git for data — and it's the right UX" connectedTo="ADR-050 (fold-section architecture)">
          <p>{"Iceberg's branchless merge (write to a branch, fast-forward the main) IS Git for data. The branch IS a copy-on-write snapshot of the table. Writes go to the branch. When ready, the branch is merged to main (atomic metadata swap). This IS the SAME pattern as Git's fast-forward merge — just for data instead of code. The pattern (branch + merge) stays; the implementation (Iceberg refs vs Git refs) changes. The user experience (write to a branch, review, merge) IS identical to Git's."}</p>
        </DeeperThought>
        <DeeperThought title="Iceberg's hidden partitioning IS the right abstraction — users never think about it" connectedTo="ADR-022 (pgvector for variant embeddings)">
          <p>{"Iceberg partitions by transformed column values (e.g., bucket(day(ts), 32) or truncate(ssn, 4)). The partition spec is part of the table metadata — not the query. The user writes SELECT * WHERE day(ts) = '2024-01-01' and Iceberg automatically prunes to the right partition. This IS the right abstraction: the user thinks in terms of the query (logical), not the partition (physical). The pattern (logical-to-physical mapping via metadata) stays; the implementation (Iceberg vs Delta vs Snowflake clustering) changes."}</p>
        </DeeperThought>
